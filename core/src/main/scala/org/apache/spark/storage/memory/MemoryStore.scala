 ...
 * 将块作为反序列化 Java对象数组 或序列化 ByteBuffers 存储在内存中。
 */
private[spark] class MemoryStore(
  ...
  private def putIterator[T](
    ...
    // How often to check whether we need to request more memory. default = 16
    ...
    /**
     * 确保我们有足够的内存来存储块。此时，块的实际内存使用量可能已经超出了展开内存的一小部分，因此我们执行最后一次调用，以尝试在必要时分配额外的内存。
     */
    if (keepUnrolling) {
