 ...
 *
 * Note: Although ExternalSorter is a fairly generic（公平通用的） sorter, some of its configuration is tied
 ...
 *
 * At a high level, this class works internally（内部的） as follows:
 *
 *  - We repeatedly(反复) fill up buffers of in-memory data（填满内存缓冲区）, using either a PartitionedAppendOnlyMap if
