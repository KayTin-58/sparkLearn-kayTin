...
/**
 * Spills contents of an in-memory collection to disk(内存集合到磁盘) when the memory threshold(阈值)
 * has been exceeded(超过).
 */
private[spark] abstract class Spillable[C](taskMemoryManager: TaskMemoryManager)
  ...
  /**
   * Force(强制) to spilling the current in-memory collection to disk to release memory,
   ...
   * memory before spilling.【在开始溢出之前尝试申请更多的内存】
   *
   * @param collection collection to spill to disk
  ...
  protected def maybeSpill(collection: C, currentMemory: Long): Boolean = {
    ...
    /** Number of elements read from input since last spill 5 * 1024 * 1024 */
    /** myMemoryThreshold = 5 * 1024 * 1024 */
    if (elementsRead % 32 == 0 && currentMemory >= myMemoryThreshold) {
      // Claim【要求】 up to double our current memory from the shuffle memory pool
    ...
    /**  numElementsForceSpillThreshold = Integer.MAX_VALUE */
